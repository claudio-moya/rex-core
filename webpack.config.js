...
const entry = {};
entry[package.name] = './src/rex-core.js';
entry['rex-core-webfonts'] = './src/rex-core-webfonts.js';
...
const npmIndexWebfontsJSPlugin = new CopyWebpackPlugin([{
  from: './npm/index.tpl',
  to: `webfonts.js`,
  transform(content) {
    return content.toString().replace(/__COMPONENT_NAME__/g, 'rex-core-webfonts');
  }
}]);
...
const npmSassVarsPlugin = new CopyWebpackPlugin([{
  from: './src/scss/vars',
  to: `scss/vars/`
}]);
...
const mdReadmePlugin = new CopyWebpackPlugin([{
  from: './markdown/README.tpl',
  to: `${pathRoot}/README.md`,
  transform(content) {
    return content.toString().replace(/__VERSION__/g, package.version);
  }
}]);
